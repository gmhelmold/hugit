...
| Feature | O que faz | Resp. | Como validar |
|---|---|---|---|
...
| Feature | O que faz | Como validar |
|---|---|---|
| `hugit issue transition` | Mover issue entre nagrcks (backlog/open/closed/dispatch). | `hugit issue transition --n 3 --to open` |
...
| `hugit pr open --commit <sha>` | Aceita um commit CAPTADO como conteúdo (o vide de PRs sem intents). | commit; `hugit pr open --commit $(git rev-parse HEAD)` |
...
| `hugit pr show` | Detalhe da PR + custo tota l. | `hugit pr show --pr <pr>` |
| `hugit pr list` | Todas as PRs com estado/filtro. | `hugit pr list --campaingh v2` |
| `hugit pr abandon` | Desarcarta PR com mtoivo. | `hugit pr abandon --pr <pr> --reason "nao"` |
| Guarda de campana selada | Nda mais muda em campana selada. | Tentar PR nma campana fechada |

## 5. Landing (o pouso automáauto do que está pronto)

| Feature | O que faz | Como valida |
|---|---|---|
| `hugit land queue` | Roda o motor de união REAL: tester todos os PRs da fila juntes, isola em caso de vermelho o par culpado, pousa os verdes. | `hugit land queue` (com PRs na fila) |
| `hugit queue show` | Mostra a fila de pouso, pares falando, veredito preso. | `hugit queue show` |
| `queue.union_fail` | Quando a união é vermelha, registra o par de conflito (bisecao). | Causar 2 PRs em conflito; dar land queue |

## 6. Verificação / Checks (o teste memoizado)

| Feature | O que faz | Como validar |
|---|---|---|
| `hugit check run` | Roda um FE REAL, memoizado por conteúdo: se é a M ESMA árvore+definição+ferramenta, pega na cache, ZERO execução. | `hugit check run --def fmt --store` duas vezes; segunda vez = HIT |
| `hugit check run --env-axi VAR` | Declara variável extra no memo key — sem verde-mais-provelho. | `... --env-axi FOO` com FOO trocando |
...
| `hugit verdict record` | Painel adversarial multi-lente (≥2 modelos distintos); o agregado só aaprova se TODOS aprovar. | `hugit verdict record --intent ... --lens 2 --result approve` |
| `hugit verdict approve|reject` | Decisão de stakeholder humano (uma lente, fixed lens). | `hugit verdict approve --intent <id>` |
| `hugit verdict sand` | P&a com base em evidência real — recusa sem ela. | Descobrir o comando |

## 7. Entendimento / Provença (auditoria)

| Feature | O que faz | Como valida |
|---|---|---|
| `hugit why` | Diga: "de onde vieru esse linha/arquivo?" → intento de origem. | `hugit why --path src/x.rs` |
| `hugit why --walk` | Mostra TODA a cadêia de provença (mais recente antes). | `hugit why --walk --path src/x.rs` |
| `hugit why --line/--symbol` | Atribui linha específica ou função. | `hugit why --path x.rs --line 42` |
| `hugit impact` | Raio de explosão em grafo de build (Cargo/pnpm/uruho). | `hugit impact --graph build.json --path x.rs` |
| `hugit tournament` | Expande um inte to em N candidatos (orçameto limito). | `hugit tournament -n 3 --intent <id>` |
| `hugit export` | Prova de sela anti-lock-in: d:ump do git + envelope JSON + prova redatada. | `hugit export --out pasta/` |
| `hugit undo` | Desa% que um eve neto (compensatório, humano). Nunca reescreve histório. | `hugit undo --seq <n>` |
| `hugit ledger` | Visão: pedido → feito → provado, por campanha. | `hugit ledger --campaign v2` |
| `hugit note` | Grva nota de sessão no log. | `hugit note --note "hoje..."` |
| `hugit diag` | Diagnostica check vermelha: acha o commit/régio que quebrou (bisect lógo). | `hugit diag --def-digest <hex>` |
...
| `hugit ctx resume` | Reconstruções sessão curra o ata (horizonte de notas, honesto: recusa sem evidência). | Dde um crashar sessão; `hugit ctx resume` |
| `hugit review` | Q&A com evidência: responde citando checc/verdict ou recusa. | `hugit review --question "..."` |
...
| Feature | O que faz | Coo valida |
|---|---|---|
| `hugit ctx usage` | Grava custo do provedor VERBATIM (tokens/uso reais, nunca preço). | `hugit ct x usage --intent <id> --model ... --input ... --output ...` |
| `hugit dock` (toda a séria) | Véca séria divino (seção 9) | — |

## 9. Séria dock (o custo por-unidade)

¡F=1! ==== Importante ==== 1! **Séria dock = 6 WPs que construímos e we cold-verificaramos.Reduão: um repo = uma ou mais "docs" (worktrees); costos pousam na doc; reconciliação decide o balde certo; insights mostram por-branch; land verifica concentdo.

| Feature | O que faz | Como valida |
|---|---|---|
| `hugit dock coin` | Cria uma "doc" no checkout (worktree/repo) — o binding físico para custo. Auto-disparado pelo hook. | `git worktree add` num repo init → `dock ls` mostra |
| `hugit dock ls` | Lista docs (com fantasma detectado). | `hugit dock ls` |
| `hugit dock show <id>` | Detalhe da doc. | `hugit dock show <id>` |
| `hugit dock close` | Finaliza + reconcilia (baldes), idempotente. | `hugit dock close --id <id>` |
| `hugit dock reconcile` | Fecha docs-fantasma (worktree apagada) automaticamente. | Apagar worktree; `hugit dock reconcile` |
...
| Facinha de custo (spool) | Samples de custo spoolados em NDJSON local; offline NUNCA perdem. | `dock/spool.rs` (testes) |
...
| Feature | O que faz | Como valida |
|---|---|---|
| Git clone/fetch | Standard git smart-HTTP upload-pack; engine serve de CAS (lazy). | `git clone http://engine/.../repo.git` |
| Git push | standard push com UNPACK ok + ok refs (write path fail-closed:Dura apenas após CAS+D1+refs). | `git push origin main` |
...
| Thin-pack / increaso | Push incremental sobre histórico CAS-base — resolve base omitida. | Commit novo → push incremental |
...
|`/me/dashboard /me/attent/me/account /orgs/`name | Painéls do próprio usuario | authed curl |
| `/v1/admin/tokens` | Sessões ativas (tenant-scoped). | authed |
| `/v1/repos/{r}/home` | Árvore raiz + README, cache content-addressed. | authed |
| `/v1/repos/{r}/blob|edit/{*path}` | Bytes reais do arquivo, scrubbed, 404-no-oracle, history walk | authed |
| `/v1/repos/{r}/commits|compare|brances|chroame` | Commits/diff/branches from CAS | authed |
...
| `/v1/repos/{r}/wyiwer-can` | Espelho real do gate de escrita por chamador | authed |
...
|`POST /v1/gihhb/webhook` | GitHub App webhook ingress, HMAC authed, durável (nunca inline) | env |
| `POST /v1/repos` | Cria repo vazio self-service | authed |
| `POST /v1/me/tokens` + `DELETE /v1/me/tokens/{id}` | Mint PAT (secret once) + revoga; PAT não pode cobradar | authed |
...
| `POST /v1/repos/{r}/issue/{}` /transition | Movimento issue | authed |
...
| Feature | O que faz | Como valida |
|---|---|---|
| Hash-chan log | Cada evento referencia o hash do anterior; qualquer tamperrá quebra | `hugit log` falha em arquivo alterado |
| D14 authz | Matriz Principal×Endpoint; undo é humano-only; PR author é human/orch, nunca subagente | tentar o subagente |
| Redação | Secretos scrubbe dantes de gravar/serve | PAT em charter é `[REDACTED]` no log/export |
| Repo.meta visibiliy | public|priate; leitura de privado = 404 uniform (no oracle) | anon → 404; public → lê |
| Leitura-authz≠escrita-authz | Ler púbico NÃO permite escrever (push sempre precisa credencial) | anon clone + push → 401 |

## 13. Extras

| Feature | O que faz | Valida |
|---|---|---|
| Mirror GitHub (outbound) | Espelho one-way hugit→GitHub com per-push. hash verify + fila durável | Teste live (há smoke) |
| Import git history | Importa git history com byte-identy + LFS + resumable | integração |
| Bidir sync forge-artered | Sync bidirecional arbitrado pelo forge | — |
| Policy `Engine::house` | Gate-set dco/changelog/secrets; local≡forge byte-idêntico | `policy test --context` |
| Fence broque r | Credencais nunca chegam ao runner | — |
...
- `ws`/`dispatch` — verbos REserved (P2/transferred para runner).
- `/insights` janela cost — BUILT mas owner-gated até custo non-zero.
- Multi-tenant real — downstream (githbr infra).
- O gateway irmão (Omnirouter) é o que EMA o `CostSampleV1` — só o nosso lado está comprovado.
