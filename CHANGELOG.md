...
- feat(serve): **`POST /v1/repos/{repo}/intents/{id}/usage` — the engine mirror of `hugit ctx usage` (cost-killer last mile, WP-1).** A new write verb records an authoring run's REAL provider token usage against a landed intent by appending a canonical `ctx.usage` record **byte-identical to the CLI's** (`target_id`/`target_kind:"intent"`/`source:"provider_usage"`/`model`/`recorded_at`/`tokens{input,output,cache_read,cache_write,total}` + optional `model_digest`, `hugit_refstore::canonical_json`) through the SAME authorized+persisted write door (`with_write`) the other verbs use — so the deployed engine can carry real per-intent token usage a companion read-fold prices into `/insights`. hugit records verbatim + prices nowhere: the only computed field is the trivially-consistent `total` (checked-add, **400 on overflow**). Fail-closed: an **unknown intent id → 404** (no phantom rows — the #113 misattribution law is structural), the id/model/digest are secret-scrubbed at the write boundary, and the D14 auth class is chain-derived (`orchestrator:`/`clerk:` → Orchestrator/Land; worker/model matrix-denied). New `crates/hugit-http-contracts/src/write_requests.rs::UsageReq` (frozen field names) + `crates/hugit-serve/src/writes/verbs/write_usage.rs` + the `["intents", id, "usage"]` route arm. Unit-tested: a well-formed request appends one canonical record (payload + total round-trip, chain still verifies), overflow → 400, unknown intent → 404, a secret-shaped model redacts.
