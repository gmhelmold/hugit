...
- feat(serve): **code search is served from a lazy, progressive, in-memory index** (W-SEARCHINDEX; design `docs/design/2026-06-29-precomputed-read-index-blob-history-and-code-search.md` P0) — retiring the per-query `O(tree × R2)` git-tree scan that wall-clock-capped a deep/large repo to partial-or-empty (the 2026-06-27 `/readyz` wedge). A new `crate::code_index::CodeIndex` rides as an engine-lifetime, interior-mutable (`Arc<RwLock<…>>`), single-thread-safe per-repo cache in `RepoState` (no new thread, no write-path change — the safe-now option vs the HA-gated R2-persistent/backfill-thread P1). On each `search`: (1) if the live HEAD root-tree changed (a push hot-swap), RESET (HEAD-invalidate — a stale tree is never served); (2) if not yet complete, advance ONE wall-clock-bounded (`SEGMENT_BUDGET` 1.5 s) + file-count-bounded (`SEGMENT_FILE_CAP` 400) build segment — walk more tree files, tokenize via `hugit_symbols::outline_blob` (symbol names + their real 1-based lines) + the filename, append postings, preserve the resumable DFS cursor; (3) query the postings built so far. After the first few queries the whole tree is indexed; thereafter every query is O(query) from memory. **Honesty + safety:** the index stores ONLY what a real walk produced (never a fabricated row) and a query is honest-partial while the build is in progress (same contract the live walk has); raw text is stored, `scrub()` is applied at the READ boundary on every emitted path + line (a redaction-rule change applies retroactively, no rebuild); bounded postings (`MAX_INDEXED_FILES` 50 000, `MAX_SYMBOLS_PER_FILE` 2 000) cap memory; per-query results are capped (`QUERY_FILE_CAP` 200, `QUERY_LINES_PER_FILE_CAP` 10) with the TRUE total reported. **Never-worse:** `build_search` falls back to the existing live bounded `search_code` git-tree grep on ANY index error (or when no index is threaded), so this is strictly additive. 14 go-live-grade tests: index-hit serves (symbol + filename), second-query-from-index, nested-tree walk, HEAD-invalidate (no stale results), progressive multi-segment build to completion, per-query cap with true total, binary-blob filename-only (no panic), case-insensitive, empty-query no-op, secret-in-an-indexed-symbol-name scrubbed at the read boundary, and the live-walk fallback.
