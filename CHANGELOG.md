...
- fix(refstore): backpressure bound honored under contention + saturating in_flight (remediation) — replaced the split admit/lock CAS (TOCTOU: in-flight could transiently exceed capacity under interleaved admission) with a non-blocking counting-semaphore gate whose permit is held from before the writer lock until after it drops, so admitted in-flight is bounded by capacity at every instant; the permit's `Drop` now uses a saturating return so a stray double-drop can never underflow the counter to `usize::MAX` and wedge admission. Added in-flight/peak instrumentation, a deterministic admit/lock-split oracle (lock pinned open → surplus rejected, peak == capacity, RED against split admission) and the high-contention peak/drain assertions in WP-D1c (wp/rrefstore-conc)
