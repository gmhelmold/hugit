...
const VECTORS: [&str; 14] = [
    ...
    // The four lease DTOs (acquire/close req+resp), mirrored byte-identical from
    // corelink-runners #230 — the conformance pass closing the 3-wire-drift history.
    "AcquireRequest.json",
    "AcquireResponse.json",
    "CloseRequest.json",
    "CloseResponse.json",
    // The agent-exec seam DTOs (cost-killer path B), mirrored byte-identical from
    // corelink-runners #290 — the exec-drive wire under the same drift tripwire.
