...
fn capture_ref_update(
    ...
    crate::runtime_store::prepare_runtime_log(log_path)
        .map_err(|e| format!("runtime migration: {}", e.to_json()))?;
    ...
    crate::runtime_store::bind_legacy_prefix(log_path, &mut log)
        .map_err(|e| format!("legacy prefix binding: {e}"))?;
...
pub fn run(args: CaptureArgs) -> ExitCode {
    ...
    let log = match crate::log_resolve::resolve_log_for_repo(args.log.clone(), &args.top_level) {
        Ok(log) => log,
        Err(e) => return err_to_log(e.to_json()),
    };
...
fn capture_commit(args: &CaptureArgs, log: &std::path::Path) -> Result<(), String> {
    let oid = args
        .oid
        .clone()
...
fn capture_checkout(args: &CaptureArgs, log: &std::path::Path) -> Result<(), String> {
    let to = args
        .oid
        .clone()
...
fn capture_push_attempt(args: &CaptureArgs, log: &std::path::Path) -> Result<(), String> {
...
fn capture_merge(args: &CaptureArgs, log: &std::path::Path) -> Result<(), String> {
    let to = args
        .oid
        .clone()
