...
//!
//! Regenerates `pnpm-lock.yaml` by running `pnpm install --frozen-lockfile`
//! (or `pnpm install` when regenerating from scratch) in the workspace root.
...
/// ```
///
/// Deterministic: pnpm lockfile generation is deterministic given the same
/// `package.json` manifests and registry snapshot.
